        ...
        <div class="container" style="margin-top: 25px;margin-left: 50px;">
            <div class="row">
                ...
                <div class="col-md-2 text-right m-b-30">
                    <router-link class="btn  btn-raised bg-grey waves-effect fa fa-chevron-circle-left" to="/admin/appointments"><strong>BACK</strong></router-link>
                </div>
            </div>
            <div class="row">
                <div class="col-md-10">
                    ...
                    <div class="col-md-10">
                        <div class="card">
                            <div class="body">
                             ...
                             <div class="row">
                                <div class="col-md-2">
                                    <button  type="button" class="btn btn-raised btn-primary m-t-15 waves-effect fa fa-send-o" @click="editAppointment()">Save Changes</button>
                                </div>
                            </div>
                        </form>
                    </div>
...
export default {
    ...
    methods:{
        ...
        sendData () {
            var self = this
            this.$http.post(apiDomain + 'api/edit_appointment',self.appointment)
            .then(response => {
        ...
        dateSelected () {
        	if(this.appointment.department === '' || this.appointment.doctor === ''){
...
created () {
	...
	this.appointment.rowID = this.$route.params.id
	this.$http.post(apiDomain + 'api/initiateAppointmentsInfo',{apt_id: self.appointment.rowID})
		.then(response => {
			if(response.status === 200){
				console.log(response)
				self.appointment.department = response.body.rowDetails[0].department
				self.departmentChanged();
				self.appointment.appointment_date = response.body.rowDetails[0].appointment_date
				self.appointment.appointment_time = response.body.rowDetails[0].time
				console.log(self.appointment.doctor)
			}
		}).catch((e) => {
			console.log(e)
		})
