...
import { FieldsDef_bill_issued } from '@/components/Pages/Admin/import_details/FieldsDef_Payments'
...
export default {
  ...
  created(){
     this.apiUrl = apiDomain + 'api/adminBill'
  ...
  methods: {
    ...
    onAction (action, data, index) {
      //console.log('slot action: ' + action, data.name, index)
        var self = this
        if(action == 'edit-item'){
           this.$router.push('/admin/update_test_issue/'+data.id)
            ...
            this.$router.push('/admin/proceed_to_payment/' + data.id)
        }
    },
    deleteData(idx){
        var self = this
       this.$http.post( apiDomain + 'api/adminBillDelete',{
          id: idx
       }).then((response)=>{
